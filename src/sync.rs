...
pub async fn run(args: Args) -> Result<()> {
    ...
    // Then by name, so that all versions of a crate are adjacent
    ...
    crates.sort_by(|crate_a, crate_b| {
        ...
        b.cmp(&a)
            .then_with(|| crate_a.name.cmp(&crate_b.name))
            .then_with(|| crate_b.version.cmp(&crate_a.version))
    });
    // Since all versions of a crate are adjacent and sorted by version, we can
    // dedup by name and be left with only the most recent version of each crate.
